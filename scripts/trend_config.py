...
# 2026-08-25 A/B: MA60 은 종가 기준으로 룰을 **완전 무력화**시켰다(외인룰 off 와 변경 0건).
# MA20 은 8건 변경·8건 전부 개선(Δ누적 +63.7%p) — 하락 초입엔 먼저 걸리고(MA20>가격),
# 급반등일엔 막아준다(MA20 이 빠르게 따라붙음). 07-31 오청산 방지 취지는 유지된다.
FOREIGN_TREND_MA = int(os.getenv("TREND_FOREIGN_TREND_MA", "20") or 0)
